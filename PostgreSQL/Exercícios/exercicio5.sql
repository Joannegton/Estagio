...
-- 1) Selecionar os pedidos dos clientes Elizabeth Melo Girão e Muriel Mondragão Vilarinho
SELECT p.id_cliente, p.id_pedido, nome_cliente, id_vendedor, i.id_produto, pd.nome_produto
FROM pedido p
join item_pedido_venda i on p.id_pedido = i.id_pedido
join produto pd on i.id_produto = pd.id_produto
JOIN usuario u ON p.id_cliente = u.id_cliente
WHERE u.nome_cliente IN ('Elizabeth Melo Girão', 'Muriel Mondragão Vilarinho')
order by p.id_cliente;

-- 2) Selecionar os pedidos realizados após a data de 02/05/2020 ordenados do mais recente para o mais antigo.
select * from pedido
where data_pedido > '2020/05/02'
order by data_pedido desc;

-- 3) Selecionar o nome, data de nascimento e origem de todos os clientes ordenados de forma alfabética e por data de nascimento
select nome_cliente, data_nasc, origem from usuario
order by nome_cliente asc, data_nasc asc;

-- 4)	Selecionar o código dos clientes que realizam pedidos sem duplicidade e ordenar
select distinct id_cliente from usuario
order by id_cliente asc;

-- 5)	Selecionar o nome e valor dos produtos que o nome contenha nokia
select nome_produto, valor_produto from produto
where nome_produto ilike '%nokia%';

--6) Selecionar código de produto todos os produtos vendidos sem duplicidade ordenados do mais antigo para o mais novo
with produtos_vendidos as ( -- Common Table Expression -  definir uma consulta temporária para ser usada em outra
    select i.id_produto, p.data_pedido
    from item_pedido_venda i
    join pedido p on i.id_pedido = p.id_pedido
)
select id_produto
from produtos_vendidos
group by id_produto -- permite aplicar função de agregação
order by min(data_pedido) asc; -- odena por ondem de data ascendente

--7) Selecionar o nome, cpf  e score dos clientes que nasceram entre 1950 e 2000 e que o nome termine com a vogal “a”
select nome_cliente, cpf, score 
from usuario 
WHERE data_nasc BETWEEN '1950-01-01' AND '2000-12-31' 
and nome_cliente ilike '%a';

--8) Selecionar o código do pedido e produto que o valor seja maior R$3000,00 e ordenar por seus códigos
select p.id_pedido, pd.nome_produto, valor_produto
from pedido p
join item_pedido_venda i on p.id_pedido = i.id_pedido
join produto pd on i.id_produto = pd.id_produto
where valor_produto > 3000;
