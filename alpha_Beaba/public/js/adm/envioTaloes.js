...
function enviarTalao(){
    ...
    const response = enviarDados('http://localhost:5000/enviarTaloes', data)
    if (response.success){
...
function carregarSelectsLojas(){
    carregarDadosSelect('lojaDestinataria', 'http://localhost:5000/lojas', 'cod_loja', 'nome_loja')
}

function carregarSelectsFuncionarios(){
    carregarDadosSelect('funcionarioRecebimento', 'http://localhost:5000/usuarios', 'matricula', 'nome_usuario')
}
